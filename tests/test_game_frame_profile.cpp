...
static std::unique_ptr<slayer3d_game_frame_profile> make_frame_profile(double budget_ms)
{
    auto profile = std::make_unique<slayer3d_game_frame_profile>();
    slayer3d_game_frame_profile_reset(profile.get(), budget_ms);
    return profile;
}

TEST(GameFrameProfileTest, SummarizesFrameDistributionAndSlowestStageBreakdown)
{
    auto profile = make_frame_profile(1000.0 / 60.0);

    slayer3d_game_frame_profile_record(profile.get(), {10.0, 1.0, 2.0, 3.0, 4.0, 1});
    slayer3d_game_frame_profile_record(profile.get(), {20.0, 2.0, 3.0, 4.0, 11.0, 1});
    slayer3d_game_frame_profile_record(profile.get(), {30.0, 3.0, 4.0, 5.0, 18.0, 2});
    slayer3d_game_frame_profile_record(profile.get(), {40.0, 4.0, 5.0, 6.0, 25.0, 3});

    slayer3d_game_frame_profile_summary summary;
    ASSERT_TRUE(slayer3d_game_frame_profile_summarize(profile.get(), &summary));
...
{
    auto profile = make_frame_profile(16.0);
    for (int i = 0; i < SLAYER3D_GAME_FRAME_PROFILE_SAMPLE_CAPACITY + 3; ++i)
        slayer3d_game_frame_profile_record(profile.get(), {20.0, 1.0, 2.0, 3.0, 14.0, 1});
...
{
    auto profile = make_frame_profile(0.0);
    slayer3d_game_frame_profile_summary summary;
    EXPECT_FALSE(slayer3d_game_frame_profile_summarize(profile.get(), &summary));
