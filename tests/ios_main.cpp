...
// os_log to the "com.slayer3d.tests" subsystem so the CI runner can tail
...
os_log_t g_log()
{
    static os_log_t log = os_log_create("com.slayer3d.tests", "tests");
    return log;
}
...
class IOSLogListener : public ::testing::EmptyTestEventListener
{
    ...
    {
        os_log_info(g_log(), "SLAYER3D_TEST RUN  %{public}s.%{public}s", info.test_suite_name(), info.name());
    }
    ...
    void OnTestPartResult(const ::testing::TestPartResult &result) override
    {
        if (result.failed())
        {
            os_log_error(g_log(), "SLAYER3D_TEST FAIL %{public}s:%d %{public}s",
    ...
    void OnTestEnd(const ::testing::TestInfo &info) override
    {
        ...
        if (failed)
        {
            os_log_error(g_log(), "SLAYER3D_TEST FAIL %{public}s.%{public}s", info.test_suite_name(), info.name());
        }
        else
        {
            os_log_info(g_log(), "SLAYER3D_TEST OK   %{public}s.%{public}s", info.test_suite_name(), info.name());
        }
    }
};
} // namespace
...
int main(int argc, char **argv)
{
    ...
    os_log_info(g_log(), "SLAYER3D_TEST_RESULT: %d", rc);
    SDL_Delay(500);
    return rc;
}
