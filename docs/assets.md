...
Slayer 3D game data should use stable virtual paths such as
`asset://scripts/rules.lua` or `asset://images/player.png`. At runtime,
`slayer3d_asset_resolver` maps those paths to mounted source directories,
`.slayer3dpak` files, or embedded memory packs.
...
slayer3d_asset_resolver_mount_pack_file(assets, "game.slayer3dpak", error, sizeof(error));
slayer3d_game_data_load_asset(assets, "asset://game.game.json", session, &runtime, error, sizeof(error));
...
`slayer3d_asset_pack_write_file()` writes deterministic packs from an explicit
...
Compression is controlled by the `SLAYER3D_COMPRESS_PACKS` CMake option. Set it
...
`SLAYER3D_PACK_PASSWORD` to an empty string to disable the wrapper. The writer adds
...
```cmake
slayer3d_add_embedded_asset_pack(my_assets
...
`my_game_assets_size`, suitable for `slayer3d_asset_resolver_mount_memory_pack()`.
The embedded bytes are generated from the same pack writer as the on-disk
`.slayer3dpak` output, so they inherit the same compression and obfuscation
settings.

For native build-time `.slayer3dpak` files:

```cmake
slayer3d_add_asset_pack(my_asset_pack
    ROOT "${CMAKE_CURRENT_SOURCE_DIR}/data"
    OUTPUT "${CMAKE_BINARY_DIR}/my_game.slayer3dpak"
    FILES
        game.game.json
        scripts/rules.lua)
```
...
`slayer3d_bundle` creates a Love2D-style fused executable after the generic runner
has already been built. It copies `slayer3d_runner`, packs game data with the same
`.slayer3dpak` writer used by the rest of the engine, appends those pack bytes to
...
If no `--file` flags are passed, `slayer3d_bundle` includes every regular file
...
```cmake
slayer3d_target_preload_asset_directory(my_target "${CMAKE_CURRENT_SOURCE_DIR}/data" "/data")
```

## Sprite Runtime Loading

`slayer3d_game_data_load_sprite_asset()` turns an authored sprite descriptor into
...
textures until `slayer3d_sprite_asset_free()`.
