...
#define EDITOR_STAIR_PANEL_ANCHOR_KEY "editor.stair.panel.anchor"
...
static void editor_stair_clear_panel_anchor(slayer3d_game_data_runtime *runtime)
{
    if (runtime == NULL || runtime->scene_state == NULL)
        return;
    slayer3d_properties_set_vec3(runtime->scene_state, EDITOR_STAIR_PANEL_ANCHOR_KEY,
                                 slayer3d_vec3_make(0.0f, 0.0f, 0.0f));
}

static void editor_stair_publish_panel_anchor(slayer3d_game_data_runtime *runtime, const brush_world_runtime *world,
                                              const editor_brush_source_box_runtime *root)
{
    ...
    slayer3d_properties_set_vec3(runtime->scene_state, EDITOR_STAIR_PANEL_ANCHOR_KEY, anchor);
}
...
void publish_editor_stair_state(slayer3d_game_data_runtime *runtime)
{
    ...
    if (!selected || root == NULL)
    {
        ...
        editor_stair_clear_panel_anchor(runtime);
        return;
    }
    ...
    editor_stair_publish_panel_anchor(runtime, world, root);
}
