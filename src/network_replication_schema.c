...
bool slayer3d_replication_field_type_from_string(const char *type, slayer3d_replication_field_type *out_type)
{
    ...
    {
        if (out_type != NULL)
            *out_type = SLAYER3D_REPLICATION_FIELD_BOOL;
        return true;
    }
    if (SDL_strcmp(type, "int32") == 0 || SDL_strcmp(type, "int") == 0)
    {
        if (out_type != NULL)
            *out_type = SLAYER3D_REPLICATION_FIELD_INT32;
        return true;
    }
    if (SDL_strcmp(type, "float32") == 0 || SDL_strcmp(type, "float") == 0)
    {
        if (out_type != NULL)
            *out_type = SLAYER3D_REPLICATION_FIELD_FLOAT32;
        return true;
    }
    if (SDL_strcmp(type, "enum_id") == 0 || SDL_strcmp(type, "string_id") == 0)
    {
        if (out_type != NULL)
            *out_type = SLAYER3D_REPLICATION_FIELD_ENUM_ID;
        return true;
    }
    if (SDL_strcmp(type, "vec2") == 0)
    {
        if (out_type != NULL)
            *out_type = SLAYER3D_REPLICATION_FIELD_VEC2;
        return true;
    }
    if (SDL_strcmp(type, "vec3") == 0)
    {
        if (out_type != NULL)
            *out_type = SLAYER3D_REPLICATION_FIELD_VEC3;
        return true;
    }
    return false;
}
                                                     ...
                                                     slayer3d_replication_field_descriptor *out_descriptor)
{
    if (out_descriptor != NULL)
    {
        ...
        out_descriptor->type = SLAYER3D_REPLICATION_FIELD_BOOL;
    }

    if (yyjson_is_str(field))
    {
        const char *path = yyjson_get_str(field);
        slayer3d_replication_field_type type = SLAYER3D_REPLICATION_FIELD_VEC3;
        if (!slayer3d_replication_builtin_field_type(path, &type))
            return false;
        if (out_descriptor != NULL)
        {
    ...
    if (path == NULL || path[0] == '\0' ||
        !slayer3d_replication_field_type_from_string(schema_json_string(field, "type"), &type))
    {
        return false;
    }
