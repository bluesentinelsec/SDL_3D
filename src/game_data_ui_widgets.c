                               ...
                               slayer3d_ui_layout_model *layout)
{
    ...
    desc.text = ui_widget_dynamic_text(runtime, node, text_buffer, sizeof(text_buffer));
    ...
    desc.selected = ui_widget_bool(node, "selected", false) ||
                    (selected_if != NULL && eval_data_condition(runtime, selected_if, NULL));
    desc.selected_index =
    ...
    for (size_t i = 0; yyjson_is_arr(children) && i < yyjson_arr_size(children); ++i)
        if (!ui_widget_add_node(runtime, yyjson_arr_get(children, i), desc.id, layout))
            return false;
    return true;
}
                                       ...
                                       slayer3d_ui_layout_model *layout)
{
    ...
    for (size_t i = 0; yyjson_is_arr(widgets) && i < yyjson_arr_size(widgets); ++i)
        if (!ui_widget_add_node(runtime, yyjson_arr_get(widgets, i), NULL, layout))
            return false;
    return true;
}

bool slayer3d_game_data_build_active_ui_widget_layout(const slayer3d_game_data_runtime *runtime, float viewport_w,
                                                      float viewport_h, slayer3d_ui_layout_model *layout)
{
    if (runtime == NULL || layout == NULL)
        return false;

    slayer3d_ui_layout_clear(layout);
    if (!ui_widget_add_root_widgets(runtime, runtime_root(runtime), layout))
        return false;
    const scene_entry *scene = active_scene_entry_const(runtime);
    if (scene != NULL && !ui_widget_add_root_widgets(runtime, scene->root, layout))
        return false;
