...
static void editor_set_brush_face_drag_state(slayer3d_game_data_runtime *runtime, bool ready, bool dragging)
{
    if (runtime == NULL || runtime->scene_state == NULL)
        return;
    slayer3d_properties_set_bool(runtime->scene_state, "editor.brush.face_drag.ready", ready);
    slayer3d_properties_set_bool(runtime->scene_state, "editor.brush.face_drag.active", dragging);
    slayer3d_properties_set_bool(runtime->scene_state, "editor.brush.face_drag.shift",
...
static bool editor_handle_brush_face_drag(slayer3d_game_data_runtime *runtime,
                                          const slayer3d_game_data_editor_selection *hover_selection,
                                          bool *out_consumed)
{
    if (out_consumed != NULL)
        *out_consumed = false;
    if (runtime == NULL || runtime->scene_state == NULL || !editor_mode_is_brush(runtime))
    {
        editor_set_brush_face_drag_state(runtime, false, false);
        return true;
    }
    ...
    if (input == NULL)
    {
        editor_set_brush_face_drag_state(runtime, can_resize_face, false);
        return true;
    }
    ...
    editor_set_brush_face_drag_state(runtime, true, true);
...
bool slayer3d_game_data_update_active_editor_tooling(slayer3d_game_data_runtime *runtime)
{
    ...
    bool brush_face_drag_consumed = false;
    if (!editor_handle_brush_face_drag(runtime, &hover_selection, &brush_face_drag_consumed))
        return false;
    if (brush_face_drag_consumed)
    {
        publish_editor_selection(runtime, outputs, &runtime->editor_active_selection);
        publish_editor_selected_brush_count(runtime);
        return true;
    ...
    if (editor_mode_is_brush(runtime) && hover_selection.hit)
    {
        runtime->editor_active_selection = hover_selection;
        runtime->editor_selection_scene = slayer3d_game_data_active_scene(runtime);
    }
