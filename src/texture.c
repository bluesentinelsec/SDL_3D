...
{
    return filter == SLAYER3D_TEXTURE_FILTER_NEAREST || filter == SLAYER3D_TEXTURE_FILTER_BILINEAR ||
           filter == SLAYER3D_TEXTURE_FILTER_TRILINEAR;
}

static bool slayer3d_texture_wrap_valid(slayer3d_texture_wrap wrap)
{
    return wrap == SLAYER3D_TEXTURE_WRAP_CLAMP || wrap == SLAYER3D_TEXTURE_WRAP_REPEAT;
}

static float slayer3d_texture_lerp(float a, float b, float t)
{
    return a + (b - a) * t;
}

static char *slayer3d_texture_strdup(const char *text)
{
...
static bool slayer3d_texture_set_defaults(slayer3d_texture2d *texture)
{
    if (texture == NULL)
    {
    ...
    texture->filter = SLAYER3D_TEXTURE_FILTER_BILINEAR;
    texture->wrap_u = SLAYER3D_TEXTURE_WRAP_CLAMP;
    texture->wrap_v = SLAYER3D_TEXTURE_WRAP_CLAMP;
    texture->mip_levels = NULL;
    texture->mip_count = 0;
    return true;
}

static void slayer3d_texture_free_mips(slayer3d_texture2d *texture)
{
...
static int slayer3d_texture_compute_mip_count(int width, int height)
{
    int levels = 1;
    int w = width;
    int h = height;
...
static bool slayer3d_texture_generate_mips(slayer3d_texture2d *texture)
{
    const int count = slayer3d_texture_compute_mip_count(texture->width, texture->height);
    slayer3d_texture_mip_level *levels =
        (slayer3d_texture_mip_level *)SDL_calloc((size_t)count, sizeof(slayer3d_texture_mip_level));
    if (levels == NULL)
    {
    ...
    {
        const slayer3d_texture_mip_level *prev = &levels[i - 1];
...
bool slayer3d_create_texture_from_image(const slayer3d_image *image, slayer3d_texture2d *out)
{
    ...
    {
        return SDL_SetError("slayer3d_create_texture_from_image requires a populated RGBA8 image.");
    }

    SDL_zerop(out);
    if (!slayer3d_texture_set_defaults(out))
    {
        return false;
    }
...
bool slayer3d_load_texture_from_file(const char *path, slayer3d_texture2d *out)
{
    ...
    SDL_zerop(&image);
    if (!slayer3d_load_image_from_file(path, &image))
    {
        return false;
    }

    ok = slayer3d_create_texture_from_image(&image, out);
...
bool slayer3d_set_texture_filter(slayer3d_texture2d *texture, slayer3d_texture_filter filter)
{
    ...
    if (!slayer3d_texture_filter_valid(filter))
    {
    ...
    {
        if (!slayer3d_texture_generate_mips(texture))
        {
            return false;
        }
    }
    else if (filter != SLAYER3D_TEXTURE_FILTER_TRILINEAR)
    {
        slayer3d_texture_free_mips(texture);
    }
...
bool slayer3d_set_texture_wrap(slayer3d_texture2d *texture, slayer3d_texture_wrap wrap_u, slayer3d_texture_wrap wrap_v)
{
    if (texture == NULL)
    {
        return SDL_InvalidParamError("texture");
    }
    if (!slayer3d_texture_wrap_valid(wrap_u) || !slayer3d_texture_wrap_valid(wrap_v))
    {
...
static float slayer3d_texture_clamp01(float value)
{
    if (value <= 0.0f)
    {
        return 0.0f;
...
static float slayer3d_texture_wrap_coord(float value, slayer3d_texture_wrap wrap)
{
    if (wrap == SLAYER3D_TEXTURE_WRAP_REPEAT)
    {
    ...
    return slayer3d_texture_clamp01(value);
}

static int slayer3d_texture_resolve_index(int coord, int extent, slayer3d_texture_wrap wrap)
{
    if (extent <= 0)
    {
        return 0;
...
static void slayer3d_texture_fetch_rgba_mip(const slayer3d_texture_mip_level *mip, int x, int y,
                                            slayer3d_texture_wrap wrap_u, slayer3d_texture_wrap wrap_v, float *out_r,
                                            float *out_g, float *out_b, float *out_a)
{
    const int ix = slayer3d_texture_resolve_index(x, mip->width, wrap_u);
    const int iy = slayer3d_texture_resolve_index(y, mip->height, wrap_v);
...
static void slayer3d_texture_sample_bilinear_mip(const slayer3d_texture_mip_level *mip, slayer3d_texture_wrap wrap_u,
                                                 slayer3d_texture_wrap wrap_v, float u, float v, float *out_r,
                                                 float *out_g, float *out_b, float *out_a)
{
    const float sample_u = slayer3d_texture_wrap_coord(u, wrap_u);
    const float sample_v = slayer3d_texture_wrap_coord(v, wrap_v);
    ...
    slayer3d_texture_fetch_rgba_mip(mip, x0, y0, wrap_u, wrap_v, &c00_r, &c00_g, &c00_b, &c00_a);
    slayer3d_texture_fetch_rgba_mip(mip, x1, y0, wrap_u, wrap_v, &c10_r, &c10_g, &c10_b, &c10_a);
    slayer3d_texture_fetch_rgba_mip(mip, x0, y1, wrap_u, wrap_v, &c01_r, &c01_g, &c01_b, &c01_a);
    slayer3d_texture_fetch_rgba_mip(mip, x1, y1, wrap_u, wrap_v, &c11_r, &c11_g, &c11_b, &c11_a);

    *out_r =
        slayer3d_texture_lerp(slayer3d_texture_lerp(c00_r, c10_r, tx), slayer3d_texture_lerp(c01_r, c11_r, tx), ty);
    *out_g =
        slayer3d_texture_lerp(slayer3d_texture_lerp(c00_g, c10_g, tx), slayer3d_texture_lerp(c01_g, c11_g, tx), ty);
    *out_b =
        slayer3d_texture_lerp(slayer3d_texture_lerp(c00_b, c10_b, tx), slayer3d_texture_lerp(c01_b, c11_b, tx), ty);
    *out_a =
        slayer3d_texture_lerp(slayer3d_texture_lerp(c00_a, c10_a, tx), slayer3d_texture_lerp(c01_a, c11_a, tx), ty);
}

static void slayer3d_texture_fetch_rgba(const slayer3d_texture2d *texture, int x, int y, float *out_r, float *out_g,
                                        float *out_b, float *out_a)
{
    const int ix = slayer3d_texture_resolve_index(x, texture->width, texture->wrap_u);
    const int iy = slayer3d_texture_resolve_index(y, texture->height, texture->wrap_v);
                                  ...
                                  float *out_g, float *out_b, float *out_a)
{
    ...
    if (texture->filter == SLAYER3D_TEXTURE_FILTER_TRILINEAR && texture->mip_levels != NULL && texture->mip_count > 1)
    {
        ...
        if (frac <= 0.0f)
        {
            slayer3d_texture_sample_bilinear_mip(&texture->mip_levels[level0], texture->wrap_u, texture->wrap_v, u, v,
        ...
        else
        {
            ...
            slayer3d_texture_sample_bilinear_mip(&texture->mip_levels[level0], texture->wrap_u, texture->wrap_v, u, v,
                                                 &r0, &g0, &b0, &a0);
            slayer3d_texture_sample_bilinear_mip(&texture->mip_levels[level1], texture->wrap_u, texture->wrap_v, u, v,
                                                 &r1, &g1, &b1, &a1);
            *out_r = slayer3d_texture_lerp(r0, r1, frac);
            *out_g = slayer3d_texture_lerp(g0, g1, frac);
            *out_b = slayer3d_texture_lerp(b0, b1, frac);
            *out_a = slayer3d_texture_lerp(a0, a1, frac);
        }
        return;
    }

    sample_u = slayer3d_texture_wrap_coord(u, texture->wrap_u);
    sample_v = slayer3d_texture_wrap_coord(v, texture->wrap_v);
    ...
    if (texture->filter == SLAYER3D_TEXTURE_FILTER_NEAREST)
    {
        const int ix = (int)SDL_floorf(texel_x + 0.5f);
        const int iy = (int)SDL_floorf(texel_y + 0.5f);
        slayer3d_texture_fetch_rgba(texture, ix, iy, out_r, out_g, out_b, out_a);
        return;
    }
    ...
    slayer3d_texture_fetch_rgba(texture, x0, y0, &c00_r, &c00_g, &c00_b, &c00_a);
    slayer3d_texture_fetch_rgba(texture, x1, y0, &c10_r, &c10_g, &c10_b, &c10_a);
    slayer3d_texture_fetch_rgba(texture, x0, y1, &c01_r, &c01_g, &c01_b, &c01_a);
    slayer3d_texture_fetch_rgba(texture, x1, y1, &c11_r, &c11_g, &c11_b, &c11_a);
...
static bool slayer3d_texture_path_is_absolute(const char *path)
{
...
static bool slayer3d_texture_path_is_asset_uri(const char *path)
{
    return path != NULL && SDL_strncmp(path, "asset://", 8) == 0;
}

static char *slayer3d_texture_dirname_dup(const char *path)
{
...
static bool slayer3d_texture_resolve_path(const char *source_path, const char *texture_path, char **out_path)
{
    char *dirname = NULL;
    ...
    if (slayer3d_texture_path_is_asset_uri(texture_path) || slayer3d_texture_path_is_absolute(texture_path))
    {
        resolved = slayer3d_texture_strdup(texture_path);
        if (resolved == NULL)
        {
            return false;
        }
    ...
    dirname = slayer3d_texture_dirname_dup(source_path);
    if (dirname == NULL)
    {
        return false;
    }
...
void slayer3d_texture_cache_destroy(slayer3d_texture_cache_entry *cache)
{
    while (cache != NULL)
    {
        slayer3d_texture_cache_entry *next = cache->next;
        SDL_free(cache->path);
        slayer3d_free_texture(&cache->texture);
        SDL_free(cache);
        cache = next;
    }
}
...
{
    return slayer3d_texture_cache_get_or_load_asset(cache, NULL, source_path, texture_path, out_texture);
}
