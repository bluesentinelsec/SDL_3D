/**
 * @file network_replication.h
 * @brief Deterministic field codec for data-driven network replication.
 *
 * The replication codec is transport independent. It writes and reads scalar
 * and vector field values using a strict little-endian wire format so higher
 * layers can build authored replication schemas without embedding per-game
 * serialization code in C.
 */

#ifndef SLAYER3D_NETWORK_REPLICATION_H
#define SLAYER3D_NETWORK_REPLICATION_H

#include <SDL3/SDL_stdinc.h>

#include <stdbool.h>
#include <stddef.h>

#include "slayer3d/types.h"

#define SLAYER3D_REPLICATION_SCHEMA_HASH_SIZE 32

#ifdef __cplusplus
extern "C"
{
#endif

    /** @brief Primitive field types supported by the replication codec. */
    typedef enum slayer3d_replication_field_type
    {
        SLAYER3D_REPLICATION_FIELD_BOOL = 1,    /**< One-byte boolean, only 0 or 1 are valid. */
        SLAYER3D_REPLICATION_FIELD_INT32 = 2,   /**< Signed 32-bit integer. */
        SLAYER3D_REPLICATION_FIELD_FLOAT32 = 3, /**< IEEE-754 32-bit float. */
        SLAYER3D_REPLICATION_FIELD_ENUM_ID = 4, /**< Signed 32-bit enum or stable string-id value. */
        SLAYER3D_REPLICATION_FIELD_VEC2 = 5,    /**< Two float32 values: x, y. */
        SLAYER3D_REPLICATION_FIELD_VEC3 = 6,    /**< Three float32 values: x, y, z. */
    } slayer3d_replication_field_type;

    /** @brief Bounded writer for replication packets. */
    typedef struct slayer3d_replication_writer
    {
        Uint8 *buffer;   /**< Destination byte buffer. */
        size_t capacity; /**< Destination capacity in bytes. */
        size_t offset;   /**< Current write offset in bytes. */
    } slayer3d_replication_writer;

    /** @brief Bounded reader for replication packets. */
    typedef struct slayer3d_replication_reader
    {
        const Uint8 *buffer; /**< Source byte buffer. */
        size_t size;         /**< Source size in bytes. */
        size_t offset;       /**< Current read offset in bytes. */
    } slayer3d_replication_reader;

    /**
     * @brief Initialize a packet writer.
     *
     * Passing NULL for @p buffer is valid only when @p capacity is zero.
     */
    void slayer3d_replication_writer_init(slayer3d_replication_writer *writer, void *buffer, size_t capacity);

    /** @brief Return the current writer byte offset, or zero for NULL. */
    size_t slayer3d_replication_writer_offset(const slayer3d_replication_writer *writer);

    /** @brief Return remaining writer capacity in bytes, or zero for NULL/invalid state. */
    size_t slayer3d_replication_writer_remaining(const slayer3d_replication_writer *writer);

    /**
     * @brief Return the wire size in bytes for a supported field type.
     *
     * Returns zero for invalid or unknown field types.
     */
    size_t slayer3d_replication_field_wire_size(slayer3d_replication_field_type type);

    /** @brief Write a one-byte replication field type tag. */
    bool slayer3d_replication_write_field_type(slayer3d_replication_writer *writer,
                                               slayer3d_replication_field_type type);

    /**
     * @brief Write raw bytes without transformation.
     *
     * This is intended for packet headers, hashes, and other schema-level
     * payloads around typed field values. Passing NULL for @p bytes is valid
     * only when @p byte_count is zero. On failure, the writer offset is
     * unchanged.
     */
    bool slayer3d_replication_write_bytes(slayer3d_replication_writer *writer, const void *bytes, size_t byte_count);

    /** @brief Write a one-byte boolean. */
    bool slayer3d_replication_write_bool(slayer3d_replication_writer *writer, bool value);

    /** @brief Write a signed 32-bit integer. */
    bool slayer3d_replication_write_int32(slayer3d_replication_writer *writer, Sint32 value);

    /** @brief Write an unsigned 32-bit integer. */
    bool slayer3d_replication_write_uint32(slayer3d_replication_writer *writer, Uint32 value);

    /** @brief Write a 32-bit float. */
    bool slayer3d_replication_write_float32(slayer3d_replication_writer *writer, float value);

    /** @brief Write a signed 32-bit enum or stable string-id value. */
    bool slayer3d_replication_write_enum_id(slayer3d_replication_writer *writer, Sint32 value);

    /** @brief Write a vec2 as two 32-bit floats. */
    bool slayer3d_replication_write_vec2(slayer3d_replication_writer *writer, slayer3d_vec2 value);

    /** @brief Write a vec3 as three 32-bit floats. */
    bool slayer3d_replication_write_vec3(slayer3d_replication_writer *writer, slayer3d_vec3 value);

    /** @brief Initialize a packet reader. */
    void slayer3d_replication_reader_init(slayer3d_replication_reader *reader, const void *buffer, size_t size);

    /** @brief Return the current reader byte offset, or zero for NULL. */
    size_t slayer3d_replication_reader_offset(const slayer3d_replication_reader *reader);

    /** @brief Return unread byte count, or zero for NULL/invalid state. */
    size_t slayer3d_replication_reader_remaining(const slayer3d_replication_reader *reader);

    /**
     * @brief Read and validate a one-byte replication field type tag.
     *
     * Invalid or unknown tags fail and leave the reader offset unchanged.
     */
    bool slayer3d_replication_read_field_type(slayer3d_replication_reader *reader,
                                              slayer3d_replication_field_type *out_type);

    /**
     * @brief Read raw bytes without transformation.
     *
     * This is intended for packet headers, hashes, and other schema-level
     * payloads around typed field values. Passing NULL for @p out_bytes is
     * valid only when @p byte_count is zero. On failure, the reader offset is
     * unchanged.
     */
    bool slayer3d_replication_read_bytes(slayer3d_replication_reader *reader, void *out_bytes, size_t byte_count);

    /**
     * @brief Read a one-byte boolean.
     *
     * Values other than 0 and 1 fail and leave the reader offset unchanged.
     */
    bool slayer3d_replication_read_bool(slayer3d_replication_reader *reader, bool *out_value);

    /** @brief Read a signed 32-bit integer. */
    bool slayer3d_replication_read_int32(slayer3d_replication_reader *reader, Sint32 *out_value);

    /** @brief Read an unsigned 32-bit integer. */
    bool slayer3d_replication_read_uint32(slayer3d_replication_reader *reader, Uint32 *out_value);

    /** @brief Read a 32-bit float. */
    bool slayer3d_replication_read_float32(slayer3d_replication_reader *reader, float *out_value);

    /** @brief Read a signed 32-bit enum or stable string-id value. */
    bool slayer3d_replication_read_enum_id(slayer3d_replication_reader *reader, Sint32 *out_value);

    /** @brief Read a vec2 encoded as two 32-bit floats. */
    bool slayer3d_replication_read_vec2(slayer3d_replication_reader *reader, slayer3d_vec2 *out_value);

    /** @brief Read a vec3 encoded as three 32-bit floats. */
    bool slayer3d_replication_read_vec3(slayer3d_replication_reader *reader, slayer3d_vec3 *out_value);

#ifdef __cplusplus
}
#endif

#endif
