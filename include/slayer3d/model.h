#ifndef SLAYER3D_MODEL_H
#define SLAYER3D_MODEL_H
...
extern "C"
{
     ...
     * choose when to realize them into slayer3d_texture2d, so scenes can be
     * loaded without touching disk twice.
     */
    typedef struct slayer3d_material
    {
        char *name;
    ...
    typedef struct slayer3d_model
    {
        slayer3d_mesh *meshes;
        int mesh_count;

        slayer3d_material *materials;
        int material_count;

        char *source_path;

        slayer3d_model_node *nodes;
        int node_count;
        int *root_nodes; /* indices into nodes[] for scene roots */
        int root_count;

        struct slayer3d_skeleton *skeleton;
        struct slayer3d_animation_clip *animations;
        int animation_count;
        ...
        slayer3d_image *embedded_textures;
