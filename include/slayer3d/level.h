 ...
 *
 * Usage:
 *   slayer3d_level_material mats[] = { ... };
 *   slayer3d_sector sectors[] = { ... };
 *   slayer3d_level level;
 *   slayer3d_build_level(sectors, count, mats, mat_count, &level);
 *   // render loop:
 *   slayer3d_draw_model(ctx, &level.model, pos, 1.0f, white);
 *   // cleanup:
 *   slayer3d_free_level(&level);
 */

#ifndef SLAYER3D_LEVEL_H
#define SLAYER3D_LEVEL_H
...
#define SLAYER3D_SECTOR_MAX_POINTS 32

    /** @brief Signal emitted by slayer3d_sector_watcher_update() when the sampled point changes sectors. */
#define SLAYER3D_SIGNAL_ENTERED_SECTOR 0x53454354

    struct slayer3d_render_context;
    struct slayer3d_asset_resolver;
    typedef struct slayer3d_signal_bus slayer3d_signal_bus;

    typedef struct slayer3d_level_material
    {
    ...
    } slayer3d_level_material;

    typedef struct slayer3d_sector
    {
        float points[SLAYER3D_SECTOR_MAX_POINTS][2]; /* XZ polygon vertices, CCW */
        int num_points;
     ...
     * remain owned by the existing slayer3d_sector definition.
     */
    typedef struct slayer3d_sector_geometry
    {
        float floor_y;         /**< Floor height at the sector centroid. */
        float ceil_y;          /**< Ceiling height at the sector centroid. */
    ...
    typedef struct slayer3d_level
    {
        slayer3d_model model;

        /* Runtime sector/portal metadata populated by slayer3d_build_level. */
        int sector_count;
    ...
    } slayer3d_level_portal;

    /* Visibility query result. */
    typedef struct slayer3d_visibility_result
    {
        bool *sector_visible; /* caller-provided array[sector_count] */
        int visible_count;    /* number of visible sectors */
    } slayer3d_visibility_result;

    /**
     * @brief Tracks sector transitions for a moving point.
     *
     * Initialize with slayer3d_sector_watcher_init(), then call
     * slayer3d_sector_watcher_update() after movement. When the sampled point
     ...
     */
    typedef struct slayer3d_sector_watcher
    {
        int current_sector;     /**< Last sector containing the point, or -1. */
        int current_ambient_id; /**< Ambient id for current_sector, or -1. */
        int entered_signal_id;  /**< Signal emitted on entry; defaults to SLAYER3D_SIGNAL_ENTERED_SECTOR. */
    } slayer3d_sector_watcher;

    typedef struct slayer3d_level_light
    {
        float position[3]; /* world XYZ */
        float color[3];    /* RGB, 0-1 */
        float intensity;
        float range;
    } slayer3d_level_light;
     ...
     */
    bool slayer3d_build_level(const slayer3d_sector *sectors, int sector_count,
     ...
     * slayer3d_sector_damage_per_second(sector) * dt. Negative sector rates and
     ...
     */
    int slayer3d_level_find_walkable_sector(const slayer3d_level *level, const slayer3d_sector *sectors, float x,
                                            float z, float feet_y, float step_height, float player_height);
     ...
     */
    int slayer3d_level_find_support_sector(const slayer3d_level *level, const slayer3d_sector *sectors, float x,
                                           float z, float feet_y, float player_height);
     ...
     */
    bool slayer3d_level_point_inside(const slayer3d_level *level, const slayer3d_sector *sectors, float x, float y,
                                     float z);
     ...
     *
     * @param watcher Watcher state initialized with slayer3d_sector_watcher_init().
     ...
     * intensity. The result is clamped to slayer3d_color range and contains
     * no ambient term — callers add their own ambient floor.
     */
    slayer3d_color slayer3d_level_sample_light(const slayer3d_level_light *lights, int light_count,
     ...
     * finds the camera's current sector, and calls slayer3d_level_compute_visibility.
     ...
     * If vis is NULL, all meshes are drawn (equivalent to slayer3d_draw_model).
     * Frustum culling still applies per-mesh as a second stage.
     */
    bool slayer3d_draw_level(struct slayer3d_render_context *context, const slayer3d_level *level,
                             const slayer3d_visibility_result *vis, slayer3d_color tint);

    /*
     * Same as slayer3d_draw_level, but resolves `asset://` material texture paths
     ...
     * embedded pack.
     */
    bool slayer3d_draw_level_with_assets(struct slayer3d_render_context *context,
                                         const struct slayer3d_asset_resolver *assets, const slayer3d_level *level,
                                         const slayer3d_visibility_result *vis, slayer3d_color tint);
