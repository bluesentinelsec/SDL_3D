 ...
 *
 * Usage:
 *   slayer3d_sprite_scene sprites;
 *   slayer3d_sprite_scene_init(&sprites);
 *   slayer3d_sprite_actor *e = slayer3d_sprite_scene_add(&sprites);
 *   e->position = slayer3d_vec3_make(5, 0, 10);
 *   e->size     = (slayer3d_vec2){3.4f, 5.2f};
 *   e->texture  = &enemy_tex;
 *   e->rotations = &enemy_rot;
 *   e->tint     = (slayer3d_color){255,255,255,255};
 *
 *   // each frame:
 *   slayer3d_sprite_scene_update(&sprites, dt);
 *   slayer3d_sprite_scene_draw(&sprites, ctx, cam_pos, &vis);
 *
 *   slayer3d_sprite_scene_free(&sprites);
 *
 * Tinting is the caller's responsibility — set each actor's tint
 * before drawing. Use slayer3d_level_sample_light from level.h as a
 * building block for point-light contribution.
 */

#ifndef SLAYER3D_SPRITE_ACTOR_H
#define SLAYER3D_SPRITE_ACTOR_H
...
#define SLAYER3D_SPRITE_ROTATION_COUNT 8
     ...
     */
    typedef struct slayer3d_sprite_rotation_set
    {
        const slayer3d_texture2d *frames[SLAYER3D_SPRITE_ROTATION_COUNT];
    } slayer3d_sprite_rotation_set;
     ...
     */
    typedef struct slayer3d_sprite_actor
    {
        slayer3d_vec3 position;
        slayer3d_vec2 size;
        const slayer3d_texture2d *texture;                    /**< Single-frame fallback texture. */
        const slayer3d_sprite_rotation_set *rotations;        /**< Base rotation set, or NULL for a single texture. */
        const slayer3d_sprite_rotation_set *animation_frames; /**< Contiguous array of animation frame rotations. */
    ...
    } slayer3d_sprite_actor;

    typedef struct slayer3d_sprite_scene
    {
        slayer3d_sprite_actor *actors;
        int count;
        int capacity;
    } slayer3d_sprite_scene;
     ...
     *
     * Yaw uses the same XZ convention as slayer3d_sprite_select_texture: 0 means
     ...
     *
     * Uses slayer3d_level_find_walkable_sector() with the actor's current Y as
     ...
     */
    const slayer3d_texture2d *slayer3d_sprite_select_texture(const slayer3d_sprite_actor *actor, float cam_x,
                                                             float cam_z);
