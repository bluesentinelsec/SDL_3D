...
static bool runner_state_apply_json_value(slayer3d_properties *props, const char *key, yyjson_val *value,
                                          char *error_buffer, int error_buffer_size)
{
    ...
    if (yyjson_is_int(value))
    {
        ...
        if (number < INT_MIN || number > INT_MAX)
            return runner_state_set_errorf(error_buffer, error_buffer_size, "state integer out of range for key '%s'",
                                           key);
        slayer3d_properties_set_int(props, key, (int)number);
        return true;
    }
    if (yyjson_is_real(value))
    {
        slayer3d_properties_set_float(props, key, (float)yyjson_get_real(value));
        return true;
    }
    if (yyjson_is_arr(value) && yyjson_arr_size(value) == 3u)
    {
        ...
        slayer3d_properties_set_vec3(
            props, key,
            slayer3d_vec3_make((float)yyjson_get_num(x), (float)yyjson_get_num(y), (float)yyjson_get_num(z)));
        return true;
    }
    if (yyjson_is_null(value))
    {
        slayer3d_properties_remove(props, key);
        return true;
    }
                                            ...
                                            int error_buffer_size)
{
    ...
    if (doc == NULL)
    {
        slayer3d_properties_set_string(props, key, raw_value);
        return true;
    }
                                           ...
                                           int error_buffer_size)
{
    ...
    const bool ok =
        slayer3d_runner_apply_state_json_object(props, (const char *)data, size, path, error_buffer, error_buffer_size);
    SDL_free(data);
    return ok;
}
