...
void slayer3d_pack_args_print_usage(const char *argv0, FILE *stream)
{
    ...
    fprintf(out, "Usage: %s --output <pack.slayer3dpak> --root <asset-root> --file <relative-path> [--file ...]\n",
            argv0 != NULL ? argv0 : "slayer3d_pack");
}
...
{
    struct arg_str *output = arg_str0(NULL, "output", "<pack.slayer3dpak>", "output pack file");
    ...
    const char *program = argc > 0 && argv != NULL && argv[0] != NULL ? argv[0] : "slayer3d_pack";
    FILE *out = stream != NULL ? stream : stderr;
    ...
    if (help->count > 0)
    {
        slayer3d_pack_args_print_usage(program, out);
        arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
