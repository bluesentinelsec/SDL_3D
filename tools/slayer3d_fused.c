/**
 * @file slayer3d_fused.c
 * @brief Footer reader/writer for SLAYER3D fused runner executables.
 */
...
enum
{
    SLAYER3D_FUSED_FOOTER_SIZE = 512,
    SLAYER3D_FUSED_MAGIC_SIZE = 16,
    SLAYER3D_FUSED_VERSION = 1,
    SLAYER3D_FUSED_PATH_OFFSET = 48,
};

static const unsigned char SLAYER3D_FUSED_MAGIC[SLAYER3D_FUSED_MAGIC_SIZE] = {
                                ...
                                int error_buffer_size)
{
    ...
    unsigned char footer[SLAYER3D_FUSED_FOOTER_SIZE];
    if (ok && file_size < SLAYER3D_FUSED_FOOTER_SIZE)
    {
        fused_set_error(error_buffer, error_buffer_size, "executable does not contain an SLAYER3D fused pack footer");
        ok = false;
    }
    if (ok && SDL_SeekIO(io, (Sint64)(file_size - SLAYER3D_FUSED_FOOTER_SIZE), SDL_IO_SEEK_SET) < 0)
    {
        fused_set_error(error_buffer, error_buffer_size, SDL_GetError());
        ok = false;
    }
    ...
    {
        fused_set_error(error_buffer, error_buffer_size, "failed to read SLAYER3D fused pack footer");
        ok = false;
    }
    SDL_CloseIO(io);
    if (!ok)
    ...
    if (SDL_memcmp(footer, SLAYER3D_FUSED_MAGIC, SLAYER3D_FUSED_MAGIC_SIZE) != 0)
    {
        fused_set_error(error_buffer, error_buffer_size, "executable does not contain an SLAYER3D fused pack footer");
        return false;
    }
    if (fused_read_u32(footer + 16) != SLAYER3D_FUSED_VERSION ||
        fused_read_u32(footer + 20) != SLAYER3D_FUSED_FOOTER_SIZE)
    {
        fused_set_error(error_buffer, error_buffer_size, "unsupported SLAYER3D fused pack footer version");
        return false;
    }
    ...
    if (path_len == 0u || path_len > SLAYER3D_FUSED_DATA_ASSET_MAX)
    {
        fused_set_error(error_buffer, error_buffer_size, "invalid SLAYER3D fused pack data asset path");
        return false;
    }
    if (pack_size == 0u || pack_offset > file_size - SLAYER3D_FUSED_FOOTER_SIZE ||
        pack_size > file_size - SLAYER3D_FUSED_FOOTER_SIZE - pack_offset || pack_offset > (uint64_t)INT64_MAX)
    {
        fused_set_error(error_buffer, error_buffer_size, "invalid SLAYER3D fused pack byte range");
        return false;
    }

    out_pack->pack_offset = pack_offset;
    out_pack->pack_size = pack_size;
    SDL_memcpy(out_pack->data_asset_path, footer + SLAYER3D_FUSED_PATH_OFFSET, path_len);
                               ...
                               const slayer3d_fused_pack *pack, char *error_buffer, int error_buffer_size)
{
    void *data = NULL;
    size_t size = 0u;
    if (!slayer3d_fused_read_pack_bytes(executable_path, pack, &data, &size, error_buffer, error_buffer_size))
        return false;
    const bool ok = slayer3d_asset_resolver_mount_memory_pack(resolver, data, size, executable_path, error_buffer,
                                                              error_buffer_size);
    SDL_free(data);
    return ok;
}
                                  ...
                                  const char *data_asset_path, char *error_buffer, int error_buffer_size)
{
    ...
    if (path_len > SLAYER3D_FUSED_DATA_ASSET_MAX)
    {
    ...
    unsigned char footer[SLAYER3D_FUSED_FOOTER_SIZE];
    SDL_zeroa(footer);
    SDL_memcpy(footer, SLAYER3D_FUSED_MAGIC, SLAYER3D_FUSED_MAGIC_SIZE);
    fused_write_u32(footer + 16, SLAYER3D_FUSED_VERSION);
    fused_write_u32(footer + 20, SLAYER3D_FUSED_FOOTER_SIZE);
    fused_write_u64(footer + 24, pack_offset);
    fused_write_u64(footer + 32, pack_size);
    fused_write_u16(footer + 40, (uint16_t)path_len);
    SDL_memcpy(footer + SLAYER3D_FUSED_PATH_OFFSET, data_asset_path, path_len);
    ...
    if (!ok)
    {
        fused_set_error(error_buffer, error_buffer_size, "failed to write SLAYER3D fused pack footer");
        return false;
    }
    return true;
}
