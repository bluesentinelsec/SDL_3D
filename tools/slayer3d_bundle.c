/**
 * @file slayer3d_bundle.c
 * @brief Build a Love2D-style fused SLAYER3D runner executable.
 */
...
{
    slayer3d_asset_pack_source *entries;
    int count;
    int capacity;
} bundle_sources;
...
static bool bundle_sources_add(bundle_sources *sources, const char *asset_path, const char *source_path)
{
    ...
    if (sources->count >= sources->capacity)
    {
        ...
        slayer3d_asset_pack_source *grown =
            (slayer3d_asset_pack_source *)SDL_realloc(sources->entries, (size_t)new_capacity * sizeof(*grown));
        if (grown == NULL)
            return false;
...
static bool bundle_collect_sources(const slayer3d_bundle_args *args, bundle_sources *sources, char *error_buffer,
...
int main(int argc, char **argv)
{
    ...
    const slayer3d_tool_cli_result cli_result = slayer3d_bundle_args_parse(argc, argv, &args, stderr);
    ...
    if (ok)
    {
        temporary_pack = bundle_append_suffix(args.output, ".tmp.slayer3dpak");
    ...
    if (ok)
        ok = slayer3d_asset_pack_write_file(temporary_pack, sources.entries, sources.count, error, (int)sizeof(error));
    ...
    if (ok)
        ok = slayer3d_fused_append_footer(args.output, pack_offset, pack_size, args.data_asset_path, error,
                                          (int)sizeof(error));
