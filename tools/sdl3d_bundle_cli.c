...
void sdl3d_bundle_args_print_usage(const char *argv0, FILE *stream)
{
    ...
    fprintf(out,
            "Usage: %s --runner <sdl3d_runner> --root <asset-root> --data <asset://game.json> --output <game> "
            "[--file <relative-path> ...]\n",
            argv0 != NULL ? argv0 : "sdl3d_bundle");
}
...
{
    struct arg_str *runner = arg_str0(NULL, "runner", "<sdl3d_runner>", "generic runner executable to copy");
    ...
    const char *program = argc > 0 && argv != NULL && argv[0] != NULL ? argv[0] : "sdl3d_bundle";
    FILE *out = stream != NULL ? stream : stderr;
    ...
    if (help->count > 0)
    {
        sdl3d_bundle_args_print_usage(program, out);
        arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
