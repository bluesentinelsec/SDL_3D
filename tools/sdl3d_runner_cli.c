...
sdl3d_tool_cli_result sdl3d_runner_args_parse(int argc, char **argv, sdl3d_runner_args *args, FILE *stream)
{
    ...
    struct arg_str *pack = arg_str0(NULL, "pack", "<game.sdl3dpak>", "mount an SDL3D asset pack");
#if defined(SDL3D_RUNNER_EMBEDDED_ASSETS)
    ...
    args->media_dir = SDL3D_MEDIA_DIR;
#endif
#if defined(SDL3D_RUNNER_DEFAULT_DATA_ASSET_PATH)
    args->data_asset_path = SDL3D_RUNNER_DEFAULT_DATA_ASSET_PATH;
#endif
#if defined(SDL3D_RUNNER_DEFAULT_EMBEDDED) && defined(SDL3D_RUNNER_EMBEDDED_ASSETS)
    args->mount_kind = SDL3D_RUNNER_MOUNT_EMBEDDED;
#endif

    const int errors = arg_parse(argc, argv, argtable);
    if (help->count > 0)
    {
        sdl3d_runner_args_print_usage(program, out);
        arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
    ...
    if (root->count > 0)
    {
        args->mount_kind = SDL3D_RUNNER_MOUNT_DIRECTORY;
        args->mount_path = root->sval[0];
    }
    else if (pack->count > 0)
    {
        args->mount_kind = SDL3D_RUNNER_MOUNT_PACK;
